<?php include "projectHandler.php"; ?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <script>
        async function apiCall(containerId, action) {
            if (containerId === 'all') {
                ...
                for (const container of containers) {
                    ...
                    try {
                        const response = await fetch('/project/api.php?containerId=' + id + '&action=' + action);
                ...
                toast.show('All containers ' + action + 'ed successfully! Reloading...', 'success');
                ...
                try {
                    const response = await fetch('/project/api.php?containerId=' + containerId + '&action=' + action);
                    const data = await response.json();
        ...
        function setProjectDescription() {
            ...
            if (description) {
                fetch('/project/api.php?action=setDescription&containerId=<?php echo htmlspecialchars($projectName); ?>', {
                    method: 'POST',
                    headers: {
